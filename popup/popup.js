...
(async function () {
  ...
  // Master
  var banana = {
    i18n: async (msg, ...placeholderValue) => {
  ...
  var UI = function () {
    (async () => {
      document.querySelector("#fetchVideosList").innerHTML = await banana.i18n(
        "si-addon-preload"
      );

      // Setup the main tabs
      this.viewTab = new OO.ui.TabPanelLayout("view", {
        label: await banana.i18n("si-popup-browse-title"),
      });
      this.historyTab = new OO.ui.TabPanelLayout("history", {
        label: await banana.i18n("si-popup-history-title"),
        classes: ["signit-popup-tab-history"],
      });
      this.paramTab = new OO.ui.TabPanelLayout("param", {
        label: await banana.i18n("si-popup-settings-title"),
        classes: ["signit-popup-tab-settings"],
      });

      // Set up the popup page layout
      this.indexLayout = new OO.ui.IndexLayout({
        autoFocus: false,
        classes: ["signit-popup-tabs"],
      });
  ...
  UI.prototype.initView = async function () {
    ...
    this.searchWidget = new SearchWidget({
      placeholder: await banana.i18n(
        "si-popup-browse-placeholder",
        JSON.stringify(Object.keys(_backgroundPage.records).length)
      ),
    });
    ...
    this.viewTab.$element
      .append(searchLayout.$element)
      .append(this.coreContent.getContainer());
  ...
  UI.prototype.changeView = async function (text) {
    ...
    const [_word, _files] = await sendMessageUp(
      "normalizeWordAndReturnFiles",
      text
    );
  ...
  UI.prototype.initHistory = async function () {
    this.$noHistory = $(
      `<div>${await banana.i18n("si-popup-history-empty")}</div>`
    );
    this.history = [];
    this.$history = [];
  ...
  UI.prototype.addHistory = function (word, store) {
    ...
    this.$history.unshift(
      $(`<a class="signit-popup-tab-history-line">${word}</a>`)
    );
    this.$history[0].on("click", this.changeView.bind(this, word));
    this.$history[0].on(
      "click",
      this.indexLayout.setTabPanel.bind(this.indexLayout, "view")
    );
  ...
  UI.prototype.initParam = async function () {
    ...
    /* Position picker */
    // Data
    HintPosition = _backgroundPage.position;
    var positionItems = [
      new OO.ui.MenuOptionWidget({
        data: "top",
        label: await banana.i18n("si-popup-settings-position-top"),
      }),
      new OO.ui.MenuOptionWidget({
        data: "bottom",
        label: await banana.i18n("si-popup-settings-position-bottom"),
      }),
    ];
    // Layout
    var positionDropdown = new OO.ui.DropdownWidget({
      label: await banana.i18n("si-popup-settings-position-dropdown"),
      menu: { items: positionItems },
      $overlay: $("body"),
    });

    var positionLayout = new OO.ui.FieldLayout(positionDropdown, {
      label: await banana.i18n("si-popup-settings-position"),
      align: "top",
      help: await banana.i18n("si-popup-settings-position-help"),
    });
    // Populate UI with correct up-to-date user's selected values
    positionDropdown
      .getMenu()
      .selectItemByData(_backgroundPage.params.position);
    positionDropdown.getMenu().on("change", (item) => {
      var selectedPosition = item.getData();
      console.log(selectedPosition);
      sendMessageUp("storeParam", ["position", selectedPosition]);
    });
    // History-logs length
    ...
    // Populate UI with correct up to date user's selected values
    // Select menus
    signLanguageDropdown
      .getMenu()
      .selectItemByData(_backgroundPage.params.signLanguage);
    uiLanguageDropdown
      .getMenu()
      .selectItemByData(_backgroundPage.params.uiLanguage);
    // Toogle buttons
    ...
    wpintegrationWidget.on("change", () =>
      sendMessageUp("storeParam", [
        "wpintegration",
        !_backgroundPage.params.wpintegration,
      ])
    );
    twospeedWidget.on("change", () =>
      sendMessageUp("storeParam", [
        "twospeed",
        !_backgroundPage.params.twospeed,
      ])
    );
    // sendMessageUp("storeParam"( 'twospeed', _backgroundPage.params.twospeed ); // twospeed in localStorage before first usage-change
    hinticonWidget.on("change", () =>
      sendMessageUp("storeParam", [
        "hinticon",
        !_backgroundPage.params.hinticon,
      ])
    );
    coloredwordsWidget.on("change", () =>
      sendMessageUp("storeParam", [
        "coloredwords",
        !_backgroundPage.params.coloredwords,
      ])
    );
    ...
    this.paramTab.$element
      ...
      .append(positionLayout.$element); // Add the new dropdown
  };
